// src/pages/LabsManagement.tsx
//
// Diagnostic lab directory — a deliberate mirror of PharmaciesManagement.tsx
// (same CRUD + approve/reject + geo picker). Keep the two in step when either
// changes. NOTE: this manages lab FACILITIES; the catalogue of orderable
// tests lives under Pharmacy & Lab Catalog -> Lab Tests.
import { useEffect, useState, useCallback } from "react";
import { Pencil, Trash2 } from "lucide-react";
import { labApi } from "../services/admin-api";
...
export default function LabsManagement() {
  ...
  const canCreate = hasPermission(PERMISSIONS.LABS_CREATE);
  const canUpdate = hasPermission(PERMISSIONS.LABS_UPDATE);
  const canApprove = hasPermission(PERMISSIONS.LABS_APPROVE);
  const canDelete = hasPermission(PERMISSIONS.LABS_DELETE);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) await labApi.update(editingId, fd);
      else await labApi.create(fd);
      setShowForm(false);
      load();
    } catch (err: any) {
  ...
  return (
    <div className="p-6">
      <PageHeader
        ...
        actions={
          canCreate && <Button onClick={openCreate}>+ Add Lab</Button>
        }
      />
      ...
      <Table>
        ...
        <TBody>
          {loading ? (
            ...
            <TableState colSpan={6}>No labs.</TableState>
          ) : (
            items.map((p) => (
              <TR key={p._id}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Pagination page={page} totalPages={totalPages} total={total} label="labs" onPageChange={setPage} />
      </div>
